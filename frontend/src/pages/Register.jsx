...
export default function Reports() {
    ...
    const handleEmailReport = async () => {
        if (!startDate || !endDate) {
            setError("Generate a report period first.");
            return;
        }
        ...
        try {
            setEmailing(true);
            setError("");
            const data = await emailPDFReport({ startDate, endDate });
            setError("");
            toast.success(
                data?.message || "Report email sent successfully."
            );
        } catch (error) {
            console.error("Report email failed:", error);
            setError(
                error?.response?.data?.error ||
                "Unable to email the report. Please try again."
            );
        } finally {
            setEmailing(false);
        }
